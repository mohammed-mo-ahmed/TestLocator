...
import { geocodePlace, GeocodingError, reverseGeocode } from "@/lib/geocoding";
import { cn } from "@/lib/cx";
import type { TestInfo, UserLocation } from "@/lib/types";
...
export default function FindMethod({
  ...
  const t = useTranslations("findMethod");
    ...
    <div className="mx-auto max-w-3xl">
      ...
      <div className="mt-10 grid gap-5 sm:grid-cols-3">
        <button
          ...
          className="tc-card-shadow flex flex-col items-start gap-4 rounded-2xl border border-slate-200 bg-white p-6 text-start transition hover:-translate-y-0.5 hover:border-emerald-300 hover:shadow-xl focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-300 disabled:cursor-wait"
        >
          ...
          <span className="block">
            <span className="block text-lg font-semibold text-slate-900">{t("gpsCta")}</span>
            <span className="mt-1 block text-sm text-slate-600">{t("gpsDesc")}</span>
          </span>
          {gps === "locating" ? (
        ...
        >
          ...
          <span className="block">
            <span className="block text-lg font-semibold text-slate-900">{t("searchCta")}</span>
            <span className="mt-1 block text-sm text-slate-600">{t("searchDesc")}</span>
          </span>
        ...
        <button
          ...
          className="tc-card-shadow flex flex-col items-start gap-4 rounded-2xl border border-slate-200 bg-white p-6 text-start transition hover:-translate-y-0.5 hover:border-violet-300 hover:shadow-xl focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-300"
        >
          ...
          <span className="block">
            <span className="block text-lg font-semibold text-slate-900">{t("centerCta")}</span>
            <span className="mt-1 block text-sm text-slate-600">{t("centerDesc")}</span>
          </span>
        </button>
      </div>
      ...
      {gps === "error" ? (
        <div className="mt-6 flex flex-col items-center gap-3 rounded-2xl border border-rose-200 bg-rose-50 px-5 py-4 text-sm text-rose-700">
          <p className="text-center">{gpsError}</p>
      ...
      <p className="mt-6 text-center text-xs text-slate-400">
        {t("testLabel", { test: test.code.toUpperCase() })}
      </p>
    </div>
  );
}